...
if ( ! function_exists( 'img_seo_batch_tick' ) ) {
	...
	function img_seo_batch_tick() {
		...
		// Safety check: prevent infinite loop if status is invalid or already done/paused
		if ( empty( $queue ) || in_array( $progress['status'], array( 'done', 'paused', 'idle' ), true ) ) {
			if ( empty( $queue ) && 'running' === $progress['status'] ) {
				$progress['status'] = 'done';
				$progress['log'][]  = '배치 완료 (큐 없음)';
			} elseif ( 'paused' === $progress['status'] ) {
				// Already paused, do not re-process
				return;
			}
